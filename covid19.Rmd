...
covid_keycountries_counts_pivoted %>%  
  ggplot() + geom_line(aes(x = Date, y = `Confirmed Counts`, color = Regions))

covid_keycountries_counts %>% pivot_longer(cols = - c('Date'), names_to = 'Region', values_to = 'Confirmed') %>% ggplot() + geom_line(aes(x = Date, y = Confirmed, color = Region) ) + facet_wrap( ~ Region)
