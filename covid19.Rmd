...
getValues <- function(n, pattern) {
  ...
  if ( pattern == " deaths " ) {
    count.vec <- bin %>% str_split(pattern = " ") %>% keep_numeric()
    count.vec <- count.vec[!is.na(count.vec)]
  } else if (pattern == " confirmed ") {
    count.vec <- bin %>% str_split(pattern = " ") %>%  extract_numeric() # depreciated but better than parse_number()
...
```{r, warning = FALSE, message = FALSE}
# troubleshooting for 3rd confirmed
n <- 25
pattern <- " deaths "
bin <- c()
...
result.n <- keyword_search(parsedsitrep[[1]],
                           keyword = pattern,
                           path = FALSE,
                           surround_lines = 0)
...
for ( i in 7:(nrow(counts)) ) {
  ...
  # reports always have 3 instances of "confirmed" metrics
  ...
  if ( instances.deaths == 1 ) { # only have china deaths
    ...
    counts$global_deaths <- current.deaths[1]
    counts$china_deaths <- current.deaths[2]
    counts$outside_china_deaths <- current.deaths[3]
  }
